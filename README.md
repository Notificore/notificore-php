# notificore-sdk
This repository contains the open source Php client for Notificor's REST API.
...
Notificore class provide access to the hlr/sms/viber clients

**Parameters**
>   **apiKey**          *(string)*     Api key from notificore.com. Required.
>   
...
>   **api source**          *(string)*        Name of your application (not required) 
    ...
    $notificore = new Notificore($apiKey, 'originator', 'aplhaname', 3, 'Bitrix-24')

## API function "balance"
Notificore provides an API to get the balance information of your account.

    $apikey = 'test_8zWaPwOaHDPRFmgIldhl';
    $api = new Notificore($apikey);
...
>   
>   **tariff**          *(int)*     Tariff code of a price grid. Use value from Notificore class if it empty. Use default value from notificore cabinet if it wasn't set at all

#### HLR request:
    
    $hlr = $notificore->getHLRClient();
...
## SMS message
  ...
  Notificore provides an API to send SMS messages to any country across the world. 

  Notificore are identified by a unique random ID. And with this ID you can always check the status of the message through the provided endpoint.

### API function sendSms

Creates a new message object. Notificore returns the created message object with each request. Per request, a max of 50 phones can be entered.
...
> (including country code) or an alphanumeric string. In case of an alphanumeric string, the maximum length is 11 characters. Uses value from Notificore if it was set. In all other cases - required 

#### Single sms
  
  
    $smsClient = $Notificore->getSmsClient();
    $smsClient->sendSms(
        '380501111111', 
        'test', 
...
> **alpha_name**  *(string)*  The sender of the message. Uses the value of "viber sender name" from Notificore class if not setted. Required if it is empty
...
> **callback_url**  *(string)*  Link for delivery reports. Reference id is required in message arrray
    ...
    $viberClient = $notificore->getViberClient();
...
> **tariff**  *(int)*  Tariff code of a price grid.
    ...
    $viberClient = $notificore->sendMessages();
...
Notificore provides an API to get the price information of your account.
